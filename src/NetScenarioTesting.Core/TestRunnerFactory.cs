...
namespace NetScenarioTesting.Core
{
    ...
    public class TestRunnerFactory
    {
        ...
        private static TestsRunnerData GetAssemblyTestTypes(Assembly assembly)
        {
            try
            {
                var testTypes = assembly.DefinedTypes.Where(x => x.GetTypeInfo().GetCustomAttribute<ScenarioTestClass>() != null).ToArray();
                ...
                var testInstanceFactory = new TestInstanceFactory(testsConfiguration?.Build());
